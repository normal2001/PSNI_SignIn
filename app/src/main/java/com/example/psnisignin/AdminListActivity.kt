...
class AdminListActivity : BaseHomeActivity() {
    ...
    private fun populateSettings() {
        ...
        findViewById<Spinner>(R.id.slackNotificationTargetInput).apply {
            adapter = ArrayAdapter(this@AdminListActivity, android.R.layout.simple_spinner_item, listOf("Channel", "Individuals")).apply {
                setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
            }
            setSelection(if (AppConfig.SLACK_NOTIFICATION_TARGET == "Individuals") 1 else 0)
        }
    }
    ...
    private fun saveAdminSettings() {
        ...
        val target = findViewById<Spinner>(R.id.slackNotificationTargetInput).selectedItem.toString()
        ...
        if (notificationsEnabled && target == "Individuals" && userGroup.isBlank()) {
            showSettingsMessage(R.id.adminMessageText, "Slack user group is required for individual notifications.", isError = true)
            return
        }
        if (notificationsEnabled && target == "Channel" && channel.isBlank()) {
        ...
        AppConfig.saveAdmin(
            ...
            slackNotificationTarget = target
        )
        if (app.databaseAvailable) SlackIntegration.refreshUsersAsync(app.database, app.logger)
