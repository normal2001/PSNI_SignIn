...
class DatabaseManager(context: Context, private val logger: AppLogger) {
    ...
    init {
        ...
        try {
            createTableIfNeeded()
            // Keep legacy cache tables intact; this cache is scoped to the configured token and group.
            db.execSQL("""CREATE TABLE IF NOT EXISTS slack_group_user_cache (
                cache_scope TEXT NOT NULL, user_id TEXT NOT NULL, display_name TEXT NOT NULL,
                PRIMARY KEY (cache_scope, user_id))""")
        } catch (e: Exception) {
            db.close()
            throw e
        }
     ...
     * Inputs: firstName, lastName, signInDateTime, reason, and visiting.
     * Optional inputs: reason and visiting may be null for imported or historical records.
     ...
     */
    @Synchronized
    fun insertSignIn(firstName: String, lastName: String, signInDateTime: String, reason: String?, visiting: String? = null): Long {
        db.beginTransaction()
        return try {
    ...
    fun getActiveForDay(date: LocalDate): List<SignInRecord> = try {
        queryRecords(
            """SELECT id, first_name, last_name, sign_in_datetime, sign_out_datetime, reason, visiting
    ...
    fun getRecord(id: Long): SignInRecord? = try {
        queryRecords(
            "SELECT id, first_name, last_name, sign_in_datetime, sign_out_datetime, reason, visiting FROM sign_in_records WHERE id = ? LIMIT 1",
            arrayOf(id.toString())
        ).firstOrNull()
    } catch (e: Exception) {
    ...
    /** Read the configured group's cached users in display-name order. */
    @Synchronized
    fun cachedSlackUsers(scope: String): List<SlackRecipient> {
        val users = mutableListOf<SlackRecipient>()
        db.rawQuery("SELECT user_id, display_name FROM slack_group_user_cache WHERE cache_scope = ? ORDER BY display_name COLLATE NOCASE, user_id", arrayOf(scope)).use { cursor ->
            while (cursor.moveToNext()) users += SlackRecipient(cursor.getString(0), cursor.getString(1))
        }
        return users
    }

    /** Replace only after every lookup succeeds; rollback preserves the previous cache. */
    @Synchronized
    fun replaceSlackUsers(scope: String, users: List<SlackRecipient>) {
        db.beginTransaction()
        try {
            db.delete("slack_group_user_cache", "cache_scope = ?", arrayOf(scope))
            users.forEach { user ->
                db.insertOrThrow("slack_group_user_cache", null, ContentValues().apply {
                    put("cache_scope", scope)
                    put("user_id", requireNotNull(user.userId))
    ...
    /** Convert the standard visitor-record columns to records, closing the cursor after use. */
        ...
        db.rawQuery(sql, args).use { cursor ->
            while (cursor.moveToNext()) {
                records += SignInRecord(
                    ...
                    visiting = if (cursor.isNull(6)) null else cursor.getString(6)
                )
            }
        }
        return records
