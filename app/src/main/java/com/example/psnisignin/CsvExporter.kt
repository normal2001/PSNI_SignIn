...
class CsvExporter(
    ...
    /** Generate a four-column current-year export from SQLite on the SMB share. */
    fun copyCurrentYearSnapshotAsync(onComplete: (success: Boolean, message: String) -> Unit) {
        executor.execute {
            try {
                ...
                logger.error("SMB_SNAPSHOT_EXPORT_FAILED", "Could not create the timestamped CSV export", e)
    ...
    /** Create a missing full yearly CSV, then write a separate export without visiting or reason. */
    private fun copyCurrentYearSnapshot(): String {
        ...
        val year = LocalDate.now().year
        val records = database.getRecordsForYear(year)
        val sourceName = "PSNI_Sign-In_Sheet_${year}.csv"
                ...
                connection.authenticate(auth).use { session ->
                    (session.connectShare(AppConfig.SMB_SHARE_NAME) as DiskShare).use { share ->
                        if (!share.fileExists(sourceName)) {
                            val csvBytes = buildCsv(records)
                                .toByteArray(StandardCharsets.UTF_8)
                            writeCsvFile(share, sourceName, csvBytes)
                        }
                        writeCsvFile(share, destinationName, buildExportCsv(records).toByteArray(StandardCharsets.UTF_8))
                    }
                }
            }
        }
     ...
     */
    internal fun buildCsv(records: List<SignInRecord>): String = serializeCsv(records, includeVisitDetails = true)

    /** Timestamped exports deliberately omit both visit-detail columns and their values. */
    internal fun buildExportCsv(records: List<SignInRecord>): String = serializeCsv(records, includeVisitDetails = false)

    private fun serializeCsv(records: List<SignInRecord>, includeVisitDetails: Boolean): String = buildString {
        append("sign_in_datetime,sign_out_datetime,first_name,last_name")
        if (includeVisitDetails) append(",visiting,reason")
        append('\n')
